...
class DINOselfsupervisedlearningModelOutput(AbstractModelOutput):
    ...
    def get_output(
        ...
        """
        output will be set to be same as the loss function
        """
    ...
    def get_out_to_loss_grad(
        ...
        """
        The output will be set to be the same as the loss function
        Therefore, the graidents will be an Identity matrix
        """
        images, labels = batch
        logits = model(images)
        # here we are directly implementing the gradient instead of relying on autodiff to do
...
TASK_TO_MODELOUT = {
    ...
    "dino_self_supervised_learning" : DINOselfsupervisedlearningModelOutput, 
}
